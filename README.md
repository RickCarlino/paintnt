...
```ts
import { Colors, PaintDocument } from "paintnt";
...
doc.state.setColor("#111111");
doc.state.setFillColor("#ffd166");
doc.state.setBackgroundColor("#f5f0e6");
...
doc.tools.curve.draw({
  from: { x: 158, y: 194 },
  to: { x: 292, y: 132 },
  control1: { x: 196, y: 110 },
  control2: { x: 244, y: 220 },
  color: "#26547c",
  thickness: 2,
});
...
doc.selection.setMask({
  bounds: { x: 178, y: 132, width: 128, height: 80 },
  isSelected(x, y) {
    const dx = x - 242;
    const dy = y - 172;
...
doc.tools.text.draw({
  x: 32,
  y: 206,
  text: "Kitchen Sink Demo",
  color: "#111111",
  background: "transparent",
});

await doc.save("kitchen-sink.bmp");
```
